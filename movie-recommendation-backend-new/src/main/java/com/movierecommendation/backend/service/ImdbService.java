...
public class ImdbService {
    ...
    public FilmDetail getFilmInfoFromTmdb(Long tmdbId) {
        ...
        ResponseEntity<FilmDetail> response = restTemplate.exchange(IMDB_API_URL + "?append_to_response=videos,credits", HttpMethod.GET, entity, FilmDetail.class, tmdbId);
    ...
    public WatchProviders getFilmProvidersFromTmdb(Long tmdbId) {
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(BEARER_TOKEN);
        HttpEntity<String> entity = new HttpEntity<>("parameters", headers);

        ResponseEntity<WatchProvidersJSONResult> response = restTemplate.exchange(IMDB_API_URL + "/watch/providers", HttpMethod.GET, entity, WatchProvidersJSONResult.class, tmdbId);
        WatchProvidersJSONResult watchProvidersJSONResult;
        WatchProviders watchProviders;

        if (response.getStatusCode().is2xxSuccessful()) {
            watchProvidersJSONResult = response.getBody();
            if (watchProvidersJSONResult == null) {
                throw new RuntimeException("Failed to fetch watch providers from TMDB for film tmdbId: " + tmdbId);
            }
            watchProviders = watchProvidersJSONResult.getResults().get("AT");
        } else {
            throw new RuntimeException("Failed to fetch watch providers from TMDB for film tmdbId: " + tmdbId);
        }
